  ...
  int currentDebugLevel = DEBUG_PRIORITY_LOW;
  ...
  void setup() {
      ...
      currentDebugLevel = DEBUG_PRIORITY_HIGH; // Set to DEBUG_PRIORITY_HIGH for testing
  ...
  void loop() {
      ...
      fadeLED(redLEDPin, 2000); // 1 second
  ...
  */
  bool updateButtonState(int buttonId, ButtonStateEnum action) {
      ...
      if (button.buttonState != action) {
          ...
          debug("Button " + String(buttonId) + " state updated to " + actionStr, DEBUG_PRIORITY_HIGH);
          debug("Button " + String(buttonId) + " prior state was " + String(button.priorButtonState == BUTTON_DOWN ? "DOWN" : "UP"), DEBUG_PRIORITY_HIGH);
          debug("Button " + String(buttonId) + " action time: " + String(actionTime), DEBUG_PRIORITY_HIGH);
          return true;
      }
      return false;
  }
  ...
  */
  bool updateControllerState(ControllerStateEnum newState) {
      ...
      if (currentControllerState.controllerState != newState) {
          debug("Attempting to update state from " + String(currentControllerState.controllerState) + " to " + String(newState), DEBUG_PRIORITY_LOW);
